...
const ReciterSelector = ({ selectedReciterId, onReciterChange }: ReciterSelectorProps) => {
  const { data: reciters, isLoading } = useReciters({ language: "ar" });
  ...
  if (isLoading) {
    return (
      <div className="flex items-center gap-3">
        ...
        <div className="w-48 h-10 bg-white/20 backdrop-blur-sm border border-white/30 rounded-md animate-pulse"></div>
      </div>
    );
  }
